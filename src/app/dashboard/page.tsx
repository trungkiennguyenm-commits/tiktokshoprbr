...
import Dashboard, { type Monthly, type Daily, type Sku, type Lapse, type Pnl, type Ship } from './Dashboard'
...
export default async function Page() {
  ...
  const [m, d, s, l, p, sh] = await Promise.all([
  ...
  const err = [m, d, s, l, p, sh].find((r) => r.error)?.error
  if (err) {
    return (
